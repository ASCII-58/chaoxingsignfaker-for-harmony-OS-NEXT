...
- Debug page at `Component/DebugPage.ets`: **long-press avatar (0.8s) or tap avatar 5x** then password `nnn` (set in pages/Index.ets; keep this doc in sync if changed). Debug page also runs the ONNX captcha slider test (`model/captcha` → `rawfile/models`, wrapper at `utils/onnx/OnnxRunner.ets`, native module at `cpp/`)
