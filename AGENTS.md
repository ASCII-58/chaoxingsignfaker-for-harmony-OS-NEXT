...
- Debug entry: **long-press avatar (0.8s) or tap avatar 5x** → pushes the `DebugPanel` route (`pages/DebugPanelPage.ets`); the in-page password gate expects `nnn` (TextInput supports Enter submit). After unlock it renders `Component/DebugPage.ets` (DB info, debug ops, 模型对比， ONNX slider test). Do NOT host DebugPage in a bindSheet — child `pushPath` pages would be hidden behind the full-screen sheet. The 模型对比 page (`pages/ModelComparePage.ets`) compares MindSpore Lite vs ONNX on `rawfile/models/test.jpg`; the `.ms` MUST be converted with `--inputDataFormat=NCHW --outputDataFormat=NCHW` (see MsCaptchaDetector.ets header) or the MS backend silently misreads the input as NHWC and drifts
